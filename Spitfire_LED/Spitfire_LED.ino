...
void patternChase()
{
  ...
  led++;
  //for the second color to go twice as fast and not just be 2 colors
  strip.setPixelColor(led2, 0, 0, 50);
  led2++;
  strip.setPixelColor(led2, 0, 0, 50);
  led2++;
  strip.show();
...
// }
else if (digitalRead(pin2) == 0 && digitalRead(pin3) == 1 && digitalRead(pin4) == 1) //flash purple
{
  strip.fill(strip.Color(0,0,0),0,maxled); //off first so the top doesnt stay a color
  delay(100);
  strip.show();
  strip.fill(strip.Color(50,0,20),0,(maxled)); //half purple 
  delay(100);

}
// else if (digitalRead(pin2) == 0 && digitalRead(pin3) == 1 && digitalRead(pin4) == 1) //half purple
// {
//   strip.fill(strip.Color(0,0,0),0,maxled); //off first so the top doesnt stay a color
//   strip.fill(strip.Color(50,0,20),0,(maxled/2)); //half purple 
// }
else if (digitalRead(pin2) == 1 && digitalRead(pin3) == 0 && digitalRead(pin4) == 1)
{
  strip.fill(strip.Color(0,0,0),0,maxled); //OFF 
}
else if(digitalRead(pin2) == 1 && digitalRead(pin3) == 1 && digitalRead(pin4) == 1)
{
  patternChase(); //color chase
}
